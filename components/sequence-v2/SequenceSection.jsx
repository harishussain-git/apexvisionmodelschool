...
const SNAP_RELEASE_FRAMES = 8
const SNAP_SETTLE_TOLERANCE = 2
const SNAP_IGNORE_SCROLL_DELTA = 1
...
function normalizeSnapConfig(snap) {
  if (snap === true) {
    return { enabled: true, direction: "both" }
  }
  ...
  return {
    ...
    direction:
      snap.direction === "up" || snap.direction === "down" || snap.direction === "both"
        ? snap.direction
        : "both",
  }
}

function getSnapAnchors(anchors = []) {
  return anchors
    .map((anchor, index) => {
      const snap = normalizeSnapConfig(anchor?.snap)
...
function isSnapDirectionAllowed(snapDirection, travelDirection) {
  if (snapDirection === "both" || travelDirection === "both") {
    return true
  }

  return snapDirection === travelDirection
}

function getSnapThresholdFrames(frameCount, snapAnchors) {
  if (snapAnchors.length < 2) {
    return clamp(Math.round(frameCount * 0.015), 3, 8)
  }

  let minGap = Number.POSITIVE_INFINITY

  for (let index = 1; index < snapAnchors.length; index += 1) {
    const previousFrame = snapAnchors[index - 1]?.currentFrame ?? 0
    const currentFrame = snapAnchors[index]?.currentFrame ?? 0
    const gap = Math.abs(currentFrame - previousFrame)
  ...
  return clamp(Math.round(minGap * 0.45), 2, 6)
}

function getSnapCandidateFromNeighbors({ currentFrame, snapAnchors, direction, thresholdFrames }) {
  let previousAnchor = null
  let nextAnchor = null

  for (const anchor of snapAnchors) {
    const anchorFrame = anchor?.currentFrame ?? 0

    if (anchorFrame <= currentFrame) {
  ...
  const candidates = [previousAnchor, nextAnchor]
    .filter(Boolean)
    .map((anchor) => ({
      anchor,
      distance: Math.abs((anchor.currentFrame ?? 0) - currentFrame),
    }))
    .filter(
      ({ anchor, distance }) =>
        distance <= thresholdFrames && isSnapDirectionAllowed(anchor.snap.direction, direction)
    )
  ...
  candidates.sort((left, right) => {
    ...
    if (direction === "down") {
      return (right.anchor.currentFrame ?? 0) - (left.anchor.currentFrame ?? 0)
    }

    if (direction === "up") {
      return (left.anchor.currentFrame ?? 0) - (right.anchor.currentFrame ?? 0)
    }

    return (left.anchor.orderIndex ?? 0) - (right.anchor.orderIndex ?? 0)
  })
...
function SequenceAnchorControls({ activeAnchor, previousAnchor, nextAnchor, onStep }) {
  return (
...
export default function SequenceSection({ data, renderAnchor }) {
  ...
  const lastScrollDirectionRef = useRef("both")
  const lastSnappedAnchorRef = useRef(null)
  const scrollStopTimeoutRef = useRef(null)
  const rafIdRef = useRef(null)
  const debugRafTimeRef = useRef(0)
  const snapAnimationFrameRef = useRef(null)
  ...
  const snapAnchors = useMemo(() => getSnapAnchors(anchors), [anchors])
  ...
  const snapThresholdFrames = useMemo(
    () => getSnapThresholdFrames(frameCount, snapAnchors),
    [frameCount, snapAnchors]
  )
  ...
  useEffect(() => {
    ...
    const syncMetricsFromScroll = () => {
      ...
      const activeAnchor = getActiveAnchorByFrame(currentFrame, anchors)
      ...
      setActiveAnchorId((current) => (current === activeAnchor?.id ? current : activeAnchor?.id ?? ""))

      const lastSnappedAnchor = lastSnappedAnchorRef.current
      if (
        lastSnappedAnchor &&
        Math.abs(currentFrame - (lastSnappedAnchor.currentFrame ?? 0)) > SNAP_RELEASE_FRAMES
      ) {
        lastSnappedAnchorRef.current = null
      }
    ...
    const animateScrollTo = (targetAnchor, options = {}) => {
      ...
      if (travelDistance <= SNAP_SETTLE_TOLERANCE) {
        lastSnappedAnchorRef.current = targetAnchor
        return
      }
    ...
    const attemptSnap = () => {
      if (!snapAnchors.length || snapStateRef.current.isSnapping) {
        return
      }
      ...
      const withinSectionRange =
        metrics.scrollY >= metrics.sectionTop - metrics.viewportHeight &&
        metrics.scrollY <= metrics.sectionTop + metrics.sectionHeight

      if (!withinSectionRange) {
        return
      }

      const candidate = getSnapCandidateFromNeighbors({
        currentFrame: metrics.currentFrame,
        snapAnchors,
        direction: lastScrollDirectionRef.current,
        thresholdFrames: snapThresholdFrames,
      })
      ...
      if (
        lastSnappedAnchor &&
        lastSnappedAnchor.id !== candidate.id &&
        Math.abs(metrics.currentFrame - (lastSnappedAnchor.currentFrame ?? 0)) <= SNAP_RELEASE_FRAMES
      ) {
        return
      }

      if (lastSnappedAnchor?.id === candidate.id) {
        return
      }

      animateScrollTo(candidate)
    }

    const scheduleSnapCheck = () => {
      clearScrollStopTimeout()
      scrollStopTimeoutRef.current = window.setTimeout(attemptSnap, SCROLL_STOP_DELAY)
    }
    ...
    const handleScroll = () => {
      ...
      if (Math.abs(delta) > SNAP_IGNORE_SCROLL_DELTA) {
        lastScrollDirectionRef.current = getDirectionFromDelta(delta)
      }
    ...
    const animate = (time) => {
      ...
      if (showDebug && time - debugRafTimeRef.current > 66) {
        ...
        setDebugState({
          ...
          snapAnchorId: snapStateRef.current.targetAnchorId ?? lastSnappedAnchorRef.current?.id ?? "",
  ...
  }, [anchors, data, frameCount, showDebug, snapAnchors, snapThresholdFrames])
      ...
      {shouldShowDebugPanel && (
        <div className="fixed bottom-3 right-3 z-50 rounded-xl border border-white/20 bg-black/70 px-3 py-2 font-mono text-[11px] text-white shadow-lg backdrop-blur-md">
          ...
          <div className="grid grid-cols-2 gap-x-3 gap-y-0.5">
            ...
            <span className="text-white/70">SnapAnchor</span>
            <span className="text-right">{debugState.snapAnchorId || "-"}</span>
